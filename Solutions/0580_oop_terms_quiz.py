...
class Building: #class definition
    ...
    def renovate(self):
        ...
        self._adjust_value(10) # Protected Method

    def _adjust_value(self, percentage): # Protected Method
        self._value *= 1 + (percentage / 100) # Protected Attribute
        print(f'Value has been adjusted by {percentage}% to {self._value:.2f}\n')


class Skyscraper(Building): #inheritance from "Building"

    def renovate(self): # Polymorphism
        print("Installing a faster elevator.")
        self._adjust_value(6) # Protected Method
